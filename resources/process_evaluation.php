...
function GetData($data, &$program_section_id)
{
	...
	foreach ($questions_answers as $qas_index => $qas_block) {
			...
			if(!empty($answer)){
				...
				if(strcmp($q_id,'AIT_E_14') == 0){
					insertCheckbox($con, $doctor_id, $program_section_id, $q_id, $answer, $longAnswer, $choice, $checkboxes, $checkboxArray);
				}

				elseif(strcmp($q_id,'bias') == 0){
					$question_id = 'AIT_E_15';
...
function insertCheckbox($con, $doctor_id, $program_section_id, $q_id, $answer, &$longAnswer, $choice, $checkboxes, &$checkboxArray){
	$checkboxArray++;
	$longAnswer .= $answer;

	if($checkboxArray == $checkboxes){
		InsertAnswers($con, $doctor_id, $program_section_id, $q_id, $longAnswer, $choice);
	}
